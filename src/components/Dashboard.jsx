...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (allPoData.length > 0) {
      console.log("Sample PO dates:", allPoData.slice(0, 5).map(po => ({
        raw: po.date,
        parsed: parseDateFromSheet(po.date),
        formatted: po.date ? format(parseDateFromSheet(po.date), "yyyy-MM-dd") : null
      })));
    }
  }, [allPoData]);
              ...
              <div className="space-y-3">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className={`w-full justify-start text-left font-normal transition-all duration-200 hover:shadow-md border-slate-200 bg-white/80 backdrop-blur-sm ${!dateRange.from && "text-muted-foreground"}`}
                    >
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                            ...
                            <TableHead className="font-bold text-slate-700">Material Name</TableHead>
                            <TableHead className="font-bold text-slate-700">Party Name</TableHead>
                            <TableHead className="font-bold text-slate-700">Truck No.</TableHead>
