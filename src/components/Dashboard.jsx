...
const parseGvizResponse = (text) => {
  ...
  console.error("No valid JSON found in response. Response format may have changed.")
...
const getStatusBadge = (status) => {
  const statusConfig = {
    Pending: {
      variant: "secondary",
      icon: Clock,
      color: "text-amber-700 bg-gradient-to-r from-amber-50 to-amber-100 border border-amber-200/50",
      glow: "shadow-lg shadow-amber-100/50",
    },
    Complete: {
      variant: "default",
      icon: CheckCircle2,
      color: "text-emerald-700 bg-gradient-to-r from-emerald-50 to-emerald-100 border border-emerald-200/50",
    ...
    "In-Transit": {
      variant: "default",
      icon: Truck,
      color: "text-blue-700 bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200/50",
  ...
  const config = statusConfig[status] || {
    variant: "secondary",
    icon: Clock,
    color: "text-slate-700 bg-gradient-to-r from-slate-50 to-slate-100 border border-slate-200/50",
    glow: "shadow-lg shadow-slate-100/50",
  }
  const Icon = config.icon

  return (
    <Badge className={`${config.color} ${config.glow} transition-all duration-300 hover:scale-110 hover:shadow-xl font-medium px-3 py-1.5`}>
...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [poRes, deliveryRes] = await Promise.all([fetch(poUrl), fetch(deliveryUrl)])

      if (!poRes.ok) throw new Error(`Failed to fetch PO sheet: ${poRes.status} ${poRes.statusText}`)
      if (!deliveryRes.ok) throw new Error(`Failed to fetch DELIVERY sheet: ${deliveryRes.status} ${deliveryRes.statusText}`)
  ...
  const filteredPoData = useMemo(() => {
    return allPoData.filter((po) => {
      ...
      if (filters.erpPoNumber && !po.erpPoNumber?.toLowerCase().includes(filters.erpPoNumber.toLowerCase()))
        return false
      if (filters.partyName !== "all" && po.partyName !== filters.partyName) return false
      if (filters.materialName !== "all" && po.materialName !== filters.materialName) return false
      if (filters.poStatus !== "all" && po.poStatus !== filters.poStatus) return false
      return true
    })
  ...
  const filteredDeliveryData = useMemo(() => {
    return allDeliveryData.filter((delivery) => {
      if (filters.erpPoNumber && !delivery.erpPoNumber?.toLowerCase().includes(filters.erpPoNumber.toLowerCase()))
        return false
      if (filters.partyName !== "all" && delivery.partyName !== filters.partyName) return false
      if (filters.materialName !== "all" && delivery.materialName !== filters.materialName) return false
      return true
    })
  ...
  const overviewData = useMemo(() => {
    ...
    kpis.totalPOs = filteredPoData.filter((po) => po.erpPoNumber && po.erpPoNumber.toString().trim() !== "").length
    ...
    filteredPoData.forEach((po) => {
      ...
      if (po.partyName) {
        partyNameCounts[po.partyName] = (partyNameCounts[po.partyName] || 0) + 1
      }

      // --- Data for Top Materials from Column D (Material Name) and sum from Column G (Qty) ---
      if (po.materialName && po.qty) {
        materialNameQuantities[po.materialName] = (materialNameQuantities[po.materialName] || 0) + po.qty
      }

      // --- Data for Top Vendors by Quantity from Column F and sum from Column G ---
      if (po.partyName && po.qty) {
        partyNameQuantities[po.partyName] = (partyNameQuantities[po.partyName] || 0) + po.qty
      }
    })
    ...
    if (poQuantityByStatus["Complete"] > 0) {
      finalPoQuantityByStatusData.push({ name: "Complete", value: poQuantityByStatus["Complete"] })
    }
  ...
  const purchaseTabTables = useMemo(() => {
    // Get PO numbers that have pending status
    const pendingPoNumbers = new Set(
      filteredPoData.filter((po) => po.poStatus === "Pending").map((po) => po.erpPoNumber),
    )

    // Pending tab: DELIVERY sheet data that corresponds to pending POs
    ...
    const inTransit = filteredDeliveryData.filter((delivery) => delivery.columnP && !delivery.columnQ)

    // Received tab: DELIVERY sheet data where both column P and column Q are not null
    const received = filteredDeliveryData.filter((delivery) => delivery.columnP && delivery.columnQ)
  ...
  const clearFilters = () => {
    setFilters({ partyName: "all", materialName: "all", poStatus: "all", erpPoNumber: "" })
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-100/40 flex items-center justify-center">
        <div className="text-center space-y-8 p-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-slate-500 font-medium">Fetching latest data from sheets...</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-indigo-100/30">
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 relative">
        {/* Enhanced Filters */}
        <Card className="mb-10 border-0 shadow-2xl bg-white/60 backdrop-blur-lg ring-1 ring-white/20">
          <CardHeader className="border-b border-slate-100/50 bg-gradient-to-r from-slate-50/80 to-white/80 backdrop-blur-sm rounded-t-lg">
            ...
            <CardDescription className="text-slate-600">Refine your data view with advanced filtering options</CardDescription>
          </CardHeader>
          <CardContent className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 items-end">
              <div className="space-y-3">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className={`w-full justify-start text-left font-normal transition-all duration-200 hover:shadow-md border-slate-200 bg-white/80 backdrop-blur-sm ${!dateRange && "text-muted-foreground"}`}
                    >
                  ...
                  <PopoverContent className="w-auto p-0 border-0 shadow-2xl bg-white/95 backdrop-blur-lg" align="start">
              ...
              <div className="space-y-3">
                ...
                <Select value={filters.partyName} onValueChange={(v) => handleFilterChange("partyName", v)}>
              ...
              <div className="space-y-3">
                ...
                <Select value={filters.poStatus} onValueChange={(v) => handleFilterChange("poStatus", v)}>
                  <SelectTrigger className="border-slate-200 bg-white/80 backdrop-blur-sm hover:shadow-md transition-all duration-200">
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleFilterChange("erpPoNumber", e.target.value)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <Card className="border-0 shadow-2xl bg-gradient-to-br from-blue-50 via-blue-100/80 to-blue-200/60 hover:shadow-3xl transition-all duration-500 transform hover:scale-105 ring-1 ring-blue-200/50">
                <CardContent className="p-8">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-bold text-blue-700 mb-2 uppercase tracking-wide">Total Purchase Orders</p>
                      <p className="text-4xl font-black text-blue-900 mb-1">{overviewData.kpis.totalPOs}</p>
              ...
              <Card className="border-0 shadow-2xl bg-gradient-to-br from-amber-50 via-amber-100/80 to-amber-200/60 hover:shadow-3xl transition-all duration-500 transform hover:scale-105 ring-1 ring-amber-200/50">
                <CardContent className="p-8">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-bold text-amber-700 mb-2 uppercase tracking-wide">Pending PO's</p>
                      <p className="text-4xl font-black text-amber-900 mb-1">{overviewData.kpis.pendingPOs}</p>
              ...
              <Card className="border-0 shadow-2xl bg-gradient-to-br from-emerald-50 via-emerald-100/80 to-emerald-200/60 hover:shadow-3xl transition-all duration-500 transform hover:scale-105 ring-1 ring-emerald-200/50">
                <CardContent className="p-8">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-bold text-emerald-700 mb-2 uppercase tracking-wide">Completed PO's</p>
                      <p className="text-4xl font-black text-emerald-900 mb-1">{overviewData.kpis.completedPOs}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50 hover:shadow-2xl transition-all duration-300">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-sm font-bold text-slate-600 mb-2 uppercase tracking-wide">Total PO Quantity</p>
              ...
              <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50 hover:shadow-2xl transition-all duration-300">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-sm font-bold text-slate-600 mb-2 uppercase tracking-wide">Total Pending PO's</p>
                  <p className="text-3xl font-black text-slate-900 mb-1">
              ...
              <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50 hover:shadow-2xl transition-all duration-300">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-sm font-bold text-slate-600 mb-2 uppercase tracking-wide">Total Completed PO's</p>
                  <p className="text-3xl font-black text-slate-900 mb-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50">
                <CardHeader className="border-b border-slate-100/50 bg-gradient-to-r from-slate-50/80 to-white/80">
                  ...
                  <CardDescription className="text-slate-600">Distribution of quantities across order statuses</CardDescription>
                </CardHeader>
                <CardContent className="p-8">
                  <div className="h-80">
                      ...
                      <RePieChart>
                        ...
                        >
                          {overviewData.poQuantityByStatusData.map((entry, index) => (
                            <Cell key={`cell-qty-status-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} />
                          ))}
                        </Pie>
                        <Legend />
                      </RePieChart>
              ...
              <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50">
                <CardHeader className="border-b border-slate-100/50 bg-gradient-to-r from-slate-50/80 to-white/80">
                  ...
                  <CardDescription className="text-slate-600">Leading suppliers by order count</CardDescription>
                </CardHeader>
                <CardContent className="p-8">
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        />
                        <Tooltip content={<CustomTooltip />} />
                        <Bar dataKey="POs" fill="#3b82f6" radius={[0, 6, 6, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50">
                <CardHeader className="border-b border-slate-100/50 bg-gradient-to-r from-slate-50/80 to-white/80">
                  ...
                  <CardDescription className="text-slate-600">Most ordered materials by quantity</CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                  <Table>
                    <TableHeader>
                      <TableRow className="border-b border-slate-100 bg-slate-50/50">
                        <TableHead className="font-bold text-slate-700">Rank</TableHead>
                    ...
                    <TableBody>
                      {overviewData.top10Materials.length > 0 ? (
                        overviewData.top10Materials.map((item, index) => (
                          <TableRow key={item.name} className="border-b border-slate-50 hover:bg-green-50/50 transition-colors duration-200">
                            <TableCell>
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-md ${index === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' :
                            ...
                            <TableCell className="text-right font-bold text-slate-900">{item.quantity.toLocaleString()}</TableCell>
                          </TableRow>
                        ))
                      ) : (
                        <TableRow>
                          <TableCell colSpan={3} className="text-center h-40 text-slate-500">
                            <div className="flex flex-col items-center justify-center space-y-3">
                              ...
                              <p className="font-semibold">No material data available</p>
              ...
              <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50">
                <CardHeader className="border-b border-slate-100/50 bg-gradient-to-r from-slate-50/80 to-white/80">
                  ...
                  <CardDescription className="text-slate-600">Leading suppliers by total quantity</CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {overviewData.top10Vendors.length > 0 ? (
                        overviewData.top10Vendors.map((item, index) => (
                          <TableRow key={item.name} className="border-b border-slate-50 hover:bg-purple-50/50 transition-colors duration-200">
                            <TableCell>
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-md ${index === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' :
                                index === 1 ? 'bg-gradient-to-br from-gray-400 to-gray-600' :
                                  index === 2 ? 'bg-gradient-to-br from-amber-600 to-amber-800' :
              ...
              <TabsContent value="pending" className="space-y-8">
                <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm ring-1 ring-slate-200/50">
                  <CardHeader className="bg-gradient-to-r from-amber-50 via-amber-100/50 to-orange-50 border-b border-amber-100/50">
                    ...
                    <CardDescription className="text-amber-700">Orders awaiting processing and issuance</CardDescription>
                  </CardHeader>
                  <CardContent className="p-0">
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                            <TableHead className="font-bold text-slate-700">Lead Time To Lift Total Qty</TableHead>
                        ...
                        <TableBody>
                          {purchaseTabTables.pending.length > 0 ? (
                            purchaseTabTables.pending.map((po) => (
                              <TableRow key={po.id} className="hover:bg-amber-50/50 border-b border-slate-100 transition-colors duration-200">
                                <TableCell className="font-medium text-slate-700">{po.leadTimeToLift || '-'}</TableCell>
                                <TableCell className="font-bold text-blue-600">{po.erpPoNumber}</TableCell>
                                <TableCell className="max-w-xs">
                                  <div className="truncate font-medium" title={po.materialName}>
                                    {po.materialName}
                                  </div>
                                </TableCell>
                                <TableCell className="font-medium text-slate-700">{po.partyName}</TableCell>
                              ...
                              <TableCell colSpan={5} className="text-center h-40 text-slate-500">
                                <div className="flex flex-col items-center justify-center space-y-4">
                                  ...
                                  <div className="text-center">
                                    <p className="font-bold text-lg text-slate-700">All Clear!</p>
                                    <p className="text-sm text-slate-500">No pending orders found in PO sheet</p>
                                  </div>
                                </div>
                              </TableCell>
                            </TableRow>
                  ...
                  <CardHeader className="bg-gradient-to-r from-blue-50 via-blue-100/50 to-indigo-50 border-b border-blue-100/50">
                    <CardTitle className="text-xl flex items-center gap-3">
                      ...
                      <Badge className="bg-blue-100 text-blue-800 border border-blue-200 ml-auto">
                        {purchaseTabTables.inTransit.length} Items
                      </Badge>
                    </CardTitle>
                    <CardDescription className="text-blue-700">Materials currently being delivered</CardDescription>
                  </CardHeader>
                  <CardContent className="p-0">
                    <div className="overflow-x-auto">
                      <Table>
                        ...
                        <TableBody>
                          {purchaseTabTables.inTransit.length > 0 ? (
                            ...
                            <TableRow>
                              <TableCell colSpan={4} className="text-center h-40 text-slate-500">
                                <div className="flex flex-col items-center justify-center space-y-4">
                                  <div className="p-4 bg-gradient-to-br from-blue-100 to-blue-200 rounded-full">
                                    <Package className="h-16 w-16 text-blue-600" />
                                  </div>
                                  <div className="text-center">
                                    <p className="font-bold text-lg text-slate-700">No Materials in Transit</p>
                                    <p className="text-sm text-slate-500">No materials currently in transit</p>
                                  </div>
                                </div>
                              </TableCell>
                            </TableRow>
                  ...
                  <CardHeader className="bg-gradient-to-r from-emerald-50 via-emerald-100/50 to-green-50 border-b border-emerald-100/50">
                    <CardTitle className="text-xl flex items-center gap-3">
                      ...
                      <Badge className="bg-emerald-100 text-emerald-800 border border-emerald-200 ml-auto">
                        {purchaseTabTables.received.length} Items
                      </Badge>
                    </CardTitle>
                    <CardDescription className="text-emerald-700">Successfully delivered and received materials</CardDescription>
                  </CardHeader>
                  <CardContent className="p-0">
                    <div className="overflow-x-auto">
                      <Table>
                              ...
                              <TableCell colSpan={4} className="text-center h-40 text-slate-500">
                                <div className="flex flex-col items-center justify-center space-y-4">
                                  <div className="p-4 bg-gradient-to-br from-slate-100 to-slate-200 rounded-full">
                                    <Archive className="h-16 w-16 text-slate-500" />
                                  </div>
                                  <div className="text-center">
                                    <p className="font-bold text-lg text-slate-700">No Received Materials</p>
                                    <p className="text-sm text-slate-500">No materials have been recorded as received yet</p>
                                  </div>
                                </div>
                              </TableCell>
                            </TableRow>
