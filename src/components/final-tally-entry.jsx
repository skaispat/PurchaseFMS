...
export default function DeliveryManagement() {
  ...
  const fetchDeliveryData = useCallback(async () => {
    ...
    try {
      ...
      const url = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(DELIVERY_SHEET)}&range=A7:AH1000&t=${cacheBuster}`
        ...
        <TableBody>
          {data.map((item) => (
            <TableRow key={item.id} className="hover:bg-muted/30">
              ...
              <TableCell>{renderLinkCell(item.billImage)}</TableCell>
            ...
            <TableRow key={item.id} className="hover:bg-muted/30">
              ...
              <TableCell className="text-sm">
                <div className="flex flex-col">
                  <span className="font-medium">{renderCell(formatTimestamp(item.timestamp))}</span>
                </div>
              </TableCell>
