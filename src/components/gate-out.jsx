...
const PENDING_COLUMNS_META = [
  ...
  { header: "Physical Image", dataKey: "physicalImageOfProduct", toggleable: true, isLink: true },
  { header: "Weight Slip Image", dataKey: "imageOfWeightSlip", toggleable: true, isLink: true },
]
          ...
          {data.map((item) => (
            <TableRow key={item.id} className="hover:bg-blue-50/50">
              {PENDING_COLUMNS_META.filter(col => visiblePendingColumns[col.dataKey]).map((column) => (
                <TableCell
                  key={column.dataKey}
                  className={`whitespace-nowrap text-xs ${column.dataKey === "liftNo" ? "font-medium text-primary" : "text-gray-700"} ${column.dataKey === "materialName" || column.dataKey === "partyName" ? "truncate max-w-[150px]" : ""}`}
                >
                        ...
                        <CardTitle className="flex items-center text-md font-semibold text-foreground">
                          ...
                          {searchQuery && (
                            <span className="text-sm font-normal text-muted-foreground ml-2">
                              - Filtered from {pendingDeliveries.length} total
                            </span>
                          )}
                        </CardTitle>
                      </div>
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <CardTitle className="flex items-center text-md font-semibold text-foreground">
                          ...
                          {searchQuery && (
                            <span className="text-sm font-normal text-muted-foreground ml-2">
                              - Filtered from {historyDeliveries.length} total
                            </span>
                          )}
                        </CardTitle>
                        <CardDescription className="text-sm text-muted-foreground mt-0.5">
                          All completed delivery receipts.
                        </CardDescription>
                      </div>
